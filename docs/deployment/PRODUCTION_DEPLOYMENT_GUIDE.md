...
> [!TIP]
> Pencadangan ini memastikan Anda memiliki *safety net* jika terjadi insiden data yang tak terduga.
...
> [!IMPORTANT]
> Skrip `docker-migrate.sh` berjalan dalam satu container terpisah *sebelum* container aplikasi utama dinyalakan. Jika migrasi gagal, deployment akan dibatalkan otomatis dan server lama tetap berjalan aman.
...
|---|---|
| ❌ `pnpm db:push` / `drizzle-kit push` | Dapat membuang atau mengubah kolom secara acak (*destructive changes*). |
