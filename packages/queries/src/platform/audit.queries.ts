...
import type { AuditLogOptions } from "@tepian-k3/types/platform/audit.types";
import {
  compareObjects,
 ...
 */
const getAuditLogById = (id: string) =>
  Effect.gen(function* () {
    const [log] = yield* Effect.tryPromise({
      try: () => db.select().from(audits).where(eq(audits.id, id)).limit(1),
      catch: (error) => {
        logError(
          "auditQueries.getAuditLogById",
