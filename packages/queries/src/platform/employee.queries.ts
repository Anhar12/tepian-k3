...
const employeeQueries = {
  ...
  getMyEmployeeProfile: (userId: string) =>
    Effect.tryPromise({
      try: () =>
        db.query.employees.findFirst({
          where: and(eq(employees.userId, userId), isNull(employees.deletedAt)),
        ...
        logError(
          "employeeQueries.getMyEmployeeProfile",
          "Failed to get my employee profile",
          { userId, error },
        );
        throw new TRPCError({
          code: "INTERNAL_SERVER_ERROR",
          message: `Gagal mengambil data profil pegawai.`,
          cause: error,
        });
      },
    }).pipe(
        ...
        try: () =>
          db
            ...
            .set({
              ...
              email: isUserExist.email,
              // ##################
              // authored (generated by gemini, Jun 11 2026 16:04 WITA)
              // ##################
              nip: input.nip,
              type: input.type,
              // ##################
