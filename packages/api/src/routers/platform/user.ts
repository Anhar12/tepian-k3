...
export const userRouter = createTRPCRouter({
  ...
  verifyUser: withAnyRole(["super_admin", "admin", "admin_pelatihan"])
    .input(
      z.object({
        userId: z.uuidv7("User ID harus berupa UUID v7"),
      }),
    )
    .mutation(async ({ input }) => {
      // ##################
      ...
      const result = await runEffect(
        Effect.gen(function* () {
          const updatedUsers = yield* usersQueries.approveUser(input.userId);
          const approvedUser = updatedUsers[0];
          if (approvedUser) {
            yield* notificationsQueries.create({
              ...
              message:
                "Akun Anda telah disetujui oleh Administrator. Sekarang Anda dapat login dan menggunakan semua layanan platform.",
            });
          }
          return approvedUser;
        }),
  ...
  rejectUser: withAnyRole(["super_admin", "admin", "admin_pelatihan"])
    .input(
      z.object({
        userId: z.uuidv7("User ID harus berupa UUID v7"),
    ...
    .mutation(async ({ input }) => {
      ...
      const result = await runEffect(
        Effect.gen(function* () {
          const updatedUsers = yield* usersQueries.rejectUser(
            input.userId,
            input.reason,
          );
          const rejectedUser = updatedUsers[0];
          if (rejectedUser) {
            yield* notificationsQueries.create({
              userId: rejectedUser.id,
              type: "general",
              title: "Verifikasi Akun Ditolak",
              message: `Pendaftaran akun Anda ditolak oleh Administrator. Alasan: ${input.reason}`,
            });
          }
          return rejectedUser;
        }),
