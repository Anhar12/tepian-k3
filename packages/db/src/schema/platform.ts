...
export const ppidDocumentCategoryEnum = pgEnum("ppid_document_category", [
  "setiap-saat",
  "berkala",
  "serta-merta",
  "dikecualikan",
...
export const ppidSubmissionStatusEnum = pgEnum("ppid_submission_status", [
  "pending",
  "disetujui",
  "ditolak",
  "selesai",
...
export const ppidDocuments = createTable(
  "ppid_documents",
  {
    id: uuid("id")
      .primaryKey()
    ...
    documentNumber: varchar("document_number", { length: 100 }),
    description: text("description"),
    category: ppidDocumentCategoryEnum("category").notNull(),
    type: varchar("type", { length: 100 }).notNull(), // e.g. "Undang-Undang", "SK"
  ...
  (table) => [
    index("ppid_doc_category_idx").using("btree", table.category),
    index("ppid_doc_title_idx").using("btree", table.title),
  ],
);

export const ppidSubmissions = createTable(
  "ppid_submissions",
  {
    id: uuid("id")
      .primaryKey()
    ...
    namaPemohon: varchar("nama_pemohon", { length: 250 }).notNull(),
    phone: varchar("phone", { length: 50 }).notNull(),
    email: varchar("email", { length: 250 }).notNull(),
    address: text("address").notNull(),
    jenisInformasi: text("jenis_informasi").notNull(),
    rincianInformasi: text("rincian_informasi").notNull(),
    tujuanPenggunaan: text("tujuan_penggunaan").notNull(),
    caraMendapatkan: varchar("cara_mendapatkan", { length: 100 }).notNull(),
    identityFileUrl: text("identity_file_url"),
    identityFileName: varchar("identity_file_name", { length: 255 }),
    identityFileSize: integer("identity_file_size"),
    identityFiles: jsonb("identity_files")
      .$type<{ url: string; name: string; size: number }[]>()
      .notNull()
      .default(sql`'[]'::jsonb`),
    status: ppidSubmissionStatusEnum("status").notNull().default("pending"),
    adminNotes: text("admin_notes"),
    responseFileUrl: text("response_file_url"),
    responseFileName: varchar("response_file_name", { length: 255 }),
    responseFileSize: integer("response_file_size"),
    userId: uuid("user_id").references(() => users.id, {
      onDelete: "set null",
    }),
    ...timestamps,
  ...
  (table) => [
    uniqueIndex("ppid_sub_ticket_idx")
      .on(table.ticketNumber)
      .where(sql`${table.deletedAt} IS NULL`),
    index("ppid_sub_email_idx").using("btree", table.email),
    index("ppid_sub_status_idx").using("btree", table.status),
  ],
);

export const mediaPublications = createTable(
  "media_publications",
  {
    id: uuid("id")
      .primaryKey()
    ...
    description: text("description"),
    content: text("content").notNull(),
    category: varchar("category", { length: 100 }).notNull().default("media"),
    isPublished: boolean("is_published").notNull().default(false),
    publishedAt: timestamp("published_at", {
      withTimezone: true,
      mode: "string",
    ...
    publisher: varchar("publisher", { length: 255 })
      .notNull()
      .default("Balai K3 Samarinda"),
    ...timestamps,
  },
  (table) => [
    index("media_publications_id_idx").using("btree", table.id),
    index("media_publications_category_idx").using("btree", table.category),
    index("media_publications_is_published_idx").using(
      "btree",
      table.isPublished,
    ),
    index("media_publications_published_at_idx").using(
      "btree",
      table.publishedAt,
    ),
  ],
