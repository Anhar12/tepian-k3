...
import type { CertificationCardData } from "./certification-card";
...
/**
 * Dialog for editing an existing employee certification.
 * Pre-populates form fields from the provided cert data.
 * File replacement is optional — if omitted, the existing file is kept.
 *
 * @param props - Component props
 * @param props.cert - The certification data to pre-populate the form
 * @param props.isOpen - Whether the dialog is open
 * @param props.setIsOpen - Callback to open/close the dialog
 */
export default function EditCertificationDialog({
  ...
  const updateMutation = useMutation(
    trpc.platform.employeeCertification.update.mutationOptions({
      onSuccess: async () => {
        await queryClient.invalidateQueries(
          trpc.platform.employeeCertification.getMyCertifications.queryOptions(),
        );
        globalSuccessToast("Sertifikasi berhasil diperbarui");
        setIsOpen(false);
      },
      onError: (error) => {
        globalErrorToast(`Gagal memperbarui sertifikasi: ${error.message}`);
      },
    }),
  );
  ...
  const handleSubmit = (
    ...
    updateMutation.mutate(toFormData(data));
  };
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Perbarui informasi sertifikasi kompetensi Anda.
          </DialogDescription>
        </DialogHeader>
            ...
            <FieldGroup>
              ...
              <Controller
                ...
                render={({ field, fieldState }) => (
                  ...
                  >
                    <FieldLabel className="ml-1 text-sm font-bold">
                      Ganti File Sertifikasi (Opsional)
                    </FieldLabel>
                    <SingleFileUpload
