...
function RouteComponent() {
  ...
  // Build a set of ready parameter IDs from worksheet items
  const readyParameterIds = useMemo(() => {
    const ids = new Set<string>();
    for (const item of worksheetItems) {
      const localUpdate = localItemUpdates.get(item.id);
      const isReady = localUpdate ? localUpdate.isReady : item.isReady;
      if (isReady) {
        ids.add(item.parameterId);
      }
    }
    return ids;
  }, [worksheetItems, localItemUpdates]);
                  ...
                  <TableBody>
                    {filteredTools.length > 0 ? (
                      filteredTools.map((tool) => {
                        const readyCount = tool.parameters.filter((p) =>
                          readyParameterIds.has(p.parameterId),
                        ).length;
