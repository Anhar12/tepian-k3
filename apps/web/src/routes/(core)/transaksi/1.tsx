...
export const Route = createFileRoute("/(core)/transaksi/1")({
  component: RouteComponent,
});

const editedFormSchema = userCompanySchema.createUserCompanySchema.extend({
  picture: z
    .union([
      z.instanceof(File),
      z.string().url(), // Allow URL strings for existing images
    ])
    .optional(),
});
...
function RouteComponent() {
  ...
  const formData = useTestingFormStore((state) => state.formData);
  const setStep1Data = useTestingFormStore((state) => state.setStep1Data);
  ...
  const form = useForm<z.infer<typeof editedFormSchema>>({
    resolver: zodResolver(editedFormSchema),
  ...
  const createUserCompanyMutation = useMutation(
    trpc.userCompany.userCreateUserCompany.mutationOptions({
      onSuccess: async (data) => {
        ...
        setStep1Data({
          ...data,
          maleWorkers: String(data.maleWorkers),
          femaleWorkers: String(data.femaleWorkers),
          picture: new File([], ""),
        });
        navigate({
          to: "/transaksi/2",
  ...
  const handleSubmit = (data: z.infer<typeof editedFormSchema>) => {
    // check if data.id exists in formData to determine if it's an create or select action
            ...
            <div className="flex w-full flex-row gap-4">
              ...
              <div className="flex flex-col items-start justify-start">
                <p className="text- text-xl font-medium">
                  Pilih atau Buat Data Perusahaan
                </p>
                <p className="text-xs text-muted-foreground">
                  Silakan pilih data perusahaan yang sudah ada atau buat data
                  baru
                </p>
              </div>
            </div>
            ...
            <Field className="space-y-1">
              ...
              <ComboBox
                ...
                onChange={(id: string) => {
                  ...
                  const selectedData = company?.find((c) => c.id === id);

                  if (selectedData) {
                    // Exclude picture-related fields from server
                    const {
                      companyPictureUrl,
                    ...
                    setStep1Data({
                      ...selectedData,
                      maleWorkers: String(selectedData.maleWorkers),
                      femaleWorkers: String(selectedData.femaleWorkers),
                      picture: new File([], ""),
                    });
                    form.reset({
                      ...rest,
                      maleWorkers: String(selectedData.maleWorkers),
                      femaleWorkers: String(selectedData.femaleWorkers),
                      picture: companyPictureUrl || new File([], ""),
                    });
                  }
                }}
